...
Page({
  ...
  selectQuizType(e) {
    this.setData({ quizType: e.currentTarget.dataset.value });
  },
  selectDifficulty(e) {
    this.setData({ difficulty: e.currentTarget.dataset.value });
  },
  toggleTeamMode() {
    this.setData({ teamMode: !this.data.teamMode });
  },
  onTeamName(e) {
    this.setData({ teamName: e.detail.value });
  },
  switchRankMode(e) {
    this.setData({ rankMode: e.currentTarget.dataset.value });
    this.loadRanks();
  },
  async startQuiz() {
  ...
  async submitQuiz() {
    ...
    try {
      const res = await api.submitQuiz({
        nickname: this.data.teamMode ? '' : '玩家',
        team: this.data.teamMode ? this.data.teamName || '队伍' : '',
        answers,
      });
      this.setData({
        quizSubmitted: true,
        quizScore: res.data.score,
        quizComment: res.data.comment,
      });
      this.loadRanks();
    } catch {
  ...
  async loadRanks() {
    try {
      const res = (await api.getRanks()) + (this.data.rankMode === 'team' ? '?mode=team' : '');
      this.setData({ ranks: res.data || [] });
    } catch {}
  },
  onIdiomInput(e) {
    this.setData({ idiomInput: e.detail.value });
  },
  async checkIdiom() {
    ...
    try {
      ...
      if (res.data.valid) {
        ...
        this.setData({
          idiomPrev: idiomInput,
          idiomChain: chain,
          idiomInput: '',
          idiomResult: '✅ 正确！',
          idiomHint: res.data.hint || '',
        });
      } else {
  ...
  onFeihuaKw(e) {
    this.setData({ feihuaKeyword: e.detail.value });
  },
  onFeihuaInput(e) {
    this.setData({ feihuaInput: e.detail.value });
  },
  async checkFeihua() {
    ...
    try {
      ...
      this.setData({
        feihuaResult: res.data.valid ? '✅ 正确！' : '❌ ' + res.data.reason,
        feihuaInput: '',
      });
    } catch {}
  },
  ...
  selectVoice(e) {
    this.setData({ voiceType: e.currentTarget.dataset.value });
  },
  async doTransform() {
  ...
  // === 视频 ===
  onVideoPrompt(e) {
    this.setData({ videoPrompt: e.detail.value });
  },
  onVideoDuration(e) {
    this.setData({ videoDuration: parseInt(e.detail.value) || 6 });
  },
  async doGenerateVideo() {
    ...
    try {
      ...
      this.setData({
        videoResult: '任务已提交！taskId: ' + (res.data.taskId || '处理中...'),
        videoTaskId: res.data.taskId,
      });
    } catch {
  ...
  // === 音乐 ===
  onMusicPrompt(e) {
    this.setData({ musicPrompt: e.detail.value });
  },
  onMusicLyrics(e) {
    this.setData({ musicLyrics: e.detail.value });
  },
  onMusicDuration(e) {
    this.setData({ musicDuration: parseInt(e.detail.value) || 30 });
  },
  async doGenerateMusic() {
    ...
    try {
      ...
      this.setData({
        musicResult: '任务已提交！taskId: ' + (res.data.taskId || '处理中...'),
        musicTaskId: res.data.taskId,
      });
    } catch {
      wx.showToast({ title: '生成失败', icon: 'none' });
    }
