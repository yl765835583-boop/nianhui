...
Page({
  ...
  selectType(e) { this.setData({ wenanType: e.currentTarget.dataset.value }); },
  selectScript(e) { this.setData({ scriptType: e.currentTarget.dataset.value }); },
  selectStyle(e) { this.setData({ style: e.currentTarget.dataset.value }); },

  onTopicInput(e) { this.setData({ topic: e.detail.value }); },
  onSongInput(e) { this.setData({ song: e.detail.value }); },
  onWordCount(e) { this.setData({ wordCount: parseInt(e.detail.value) || 500 }); },
  onPeople(e) { this.setData({ people: parseInt(e.detail.value) || 3 }); },
  onDuration(e) { this.setData({ duration: parseInt(e.detail.value) || 5 }); },

  async doGenerate() {
    this.setData({ loading: true, showResult: false });
    try {
      const { mode, wenanType, scriptType, style, topic, wordCount, people, duration, song } = this.data;
      let res;
      if (mode === 'wenan') {
