...
router.get('/pool', (req, res) => {
  ...
  const pool = data.pool.filter(
    (p) => !data.drawn.includes(p.id) && !data.blacklist.includes(p.name)
  );
  res.json(ok(pool));
});
...
router.post('/admin/import', (req, res) => {
  ...
  const data = storage.getLottery();
  data.pool = names.map((n, i) => ({
    id: 'p' + i,
    name: typeof n === 'string' ? n : n.name,
    dept: typeof n === 'string' ? '' : n.dept || '',
  }));
  data.winners = [];
  data.drawn = [];
...
router.post('/draw', async (req, res) => {
  ...
  const remaining = data.pool.filter(
    (p) => !data.drawn.includes(p.id) && !data.blacklist.includes(p.name)
  );
  ...
  for (const w of drawn) {
    let msg = '';
    try {
      msg = await chatLLM('为' + w.name + '生成一段年会中奖幸运寄语（30字以内）', {
        style: '幽默轻松',
      });
    } catch {
      /* 寄语失败时保持空串 */
