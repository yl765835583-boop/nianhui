...
async function callMiniMaxTTS(provider, text, voiceType) {
  ...
  const data = await res.json();
  if (data.base_resp && data.base_resp.status_code !== 0 && data.base_resp.status_code !== 20000000) {
  ...
  const audioUrl = audioHex ? `data:audio/mp3;base64,${Buffer.from(audioHex, 'hex').toString('base64')}` : '';
  return { url: audioUrl, msg: `[MiniMax] ${voiceType}: ${text.slice(0, 30)}...`, provider: 'minimax' };
}
async function chatLLM(prompt, options = {}) {
  ...
  const messages = [
    { role: 'system', content: `你是一个专业的年会文案创作助手。回答风格：${style}。请生成高质量、可直接使用的内容。` },
    { role: 'user', content: prompt },
  ];

  const result = await routeWithFallback(cfg.providers, async (provider) => {
...
function getVoiceParams(voiceType) {
  const map = {
    '萝莉': { voice_name: 'xiaoyan', speed: 50, pitch: 70, volume: 50 },
    '大叔': { voice_name: 'xiaofeng', speed: 40, pitch: 30, volume: 60 },
    '机器人': { voice_name: 'xiaoxin', speed: 30, pitch: 50, volume: 60 },
    '财神音': { voice_name: 'laosun', speed: 50, pitch: 50, volume: 70 },
  };
  return map[voiceType] || map['萝莉'];
}
...
async function synthesizeVoice(text, voiceType = '萝莉') {
  ...
  }, cfg.fallbackToMock, 'voice');

  if (result !== null) return result;
  return { url: '', msg: 'Mock: 已模拟合成 [' + voiceType + '] 音色: ' + text.slice(0, 20) + '...' };
}
...
const mockCopywriting = {
  ...
  '颁奖词': {
    '正式大气': '他用汗水浇灌梦想，用实干诠释担当。在过去的一年里，他是团队的中流砥柱，用卓越的业绩交出了一份亮眼的答卷。他就是——年度优秀员工奖获得者！'
  },
  '抽奖话术': {
    '幽默轻松': '激动人心的时刻到了！现在要抽取的是今晚的三等奖！听说中奖率高达……呃，反正比找对象的概率高！来，让我们看看大屏幕——'
  },
  '年会标语': {
...
const personaWords = ['披荆斩棘','乘风破浪','开天辟地','稳如泰山','锋芒毕露','深藏不露','笑傲江湖','一马当先','默默奉献','闪闪发光','后起之秀','中流砥柱','创意无限','效率达人','团队之光','卷王之王'];
const personaTags = ['会议终结者','咖啡续命师','PPT魔法师','Deadline战神','团建气氛组','报销单收藏家','打印机杀手','外卖品鉴官','摸鱼艺术家','加班守夜人','零食黑洞','欢乐喜剧人'];
const luckyQuotes = [
...
function mockTextResponse(prompt, style) {
  ...
  if (prompt.includes('年度词') || prompt.includes('年度总结') || prompt.includes('人设')) {
    ...
    return '✨ 年度关键词：**' + word + '**\n🏷️ 趣味标签：**' + tag + '**\n📝 年度总结：这一年，你' + word + '，在平凡中创造了不凡。每一个努力的日子都值得被记住，新的一年继续发光发热！';
  }
  if (prompt.includes('幸运') || prompt.includes('中奖')) {
    return luckyQuotes[Math.floor(Math.random() * luckyQuotes.length)];
  }
  return '【Mock】AI 生成的回答。\n提示：' + prompt.slice(0, 100) + '\n风格：' + style + '\n\n配置 API Key 即可接入真实大模型（DeepSeek/通义/文心/智谱/豆包/Kimi）。';
}

function mockImageResponse(style) {
  const colors = { '财神': 'FF4444','古风': '8B7355','Q版': 'FFB6C1','职场潮人': '4169E1','年会礼服': 'FFD700' };
  const color = (colors[style] || 'CCCCCC');
  return {
    url: 'https://via.placeholder.com/512/' + color + '/fff?text=' + encodeURIComponent(style + '头像'),
    style
  };
}
...
async function generateVideo(prompt, options = {}) {
  const { duration = 5, style = '年会喜庆' } = options;
  ...
  }, cfg.fallbackToMock, 'video');

  if (result !== null) return result;
  return { url: '', taskId: '', msg: 'Mock: 视频生成模拟完成（配置 MiniMax Key 即可生成真实视频）' };
}
...
async function generateMusic(prompt, options = {}) {
  const { lyrics = '', duration = 30, style = '欢快' } = options;
